...
mod tests {
    ...
    fn default_curve() -> SmoothBondingCurve {
        SmoothBondingCurve {
            a_total_tokens: xyber_params::A_TOTAL_TOKENS,
            k_virtual_pool_offset: xyber_params::K_VIRTUAL_POOL_OFFSET,
            c_bonding_scale_factor: xyber_params::C_BONDING_SCALE_FACTOR,
        }
    }
    ...
    fn test_buy_exact_input() {
        ...
        let base_in = (0.01_f64 * LAMPORTS_PER_SOL as f64) as u64;
    ...
    fn test_buy_various_inputs() {
        ...
        let mut prev_minted = u128::MIN;
        let curve = default_curve();

        for base_token in base_in {
            // each iteration starts a new test scenario with old_x=0
            let old_x_local = 0;

            let lamports_in = (LAMPORTS_PER_SOL as f64 * base_token) as u64;
            let (minted, _new_x) = curve.buy_exact_input(old_x_local, lamports_in).unwrap();

            println!(
                "base_token in = {:.4}, lamports_in = {}, minted = {}",
                base_token, lamports_in, minted
            );
